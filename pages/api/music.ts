...
export const fetchLastPlayedTrack = async () => {
  await getNewAccessTokenFromRefreshToken(process.env.SPOTIFY_REFRESH_TOKEN!);
  ...
  if (!data.item) {
    const { items }: { items: { track: SpotifyTrack; played_at: string }[] } =
      await fetcher(`https://api.spotify.com/v1/me/player/recently-played`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${process.env.SPOTIFY_ACCESS_TOKEN!}`,
        },
