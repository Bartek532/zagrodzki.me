...
export const PostThumbnail = ({ post, className }: PostThumbnailProps) => (
  <SuperLink href={`/blog/${post.slug}`} key={post.slug}>
    <motion.article
      className={cn(
        "flex flex-col justify-between gap-8 bg-background p-6 transition-colors hover:bg-card dark:hover:bg-accent",
        "sm:p-8",
        className,
      )}
      layout
    >
      <div className="flex flex-col gap-2">
        ...
        <Prose>
          <p className="text-pretty leading-normal">{post.excerpt}</p>
        </Prose>
